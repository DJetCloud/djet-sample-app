...
class ProductLinesApiIT : AbstractIntegrationTest<ProductLine>() {
	...
	@Test
	fun `productLinesDeleteProductLine api`() {
		val res = createWithRequiredFields()
	...
	@Test
	fun `productLinesGetProductLine with required fields`() {
		val res = createWithRequiredFields()
		val savedRes = repository.save(res)
	...
	@Test
	fun `productLinesGetProductLine with all fields`() {
		val res = createWithAllFields()
		val savedRes = repository.save(res)
	...
	@Test
	fun `productLinesGetProductLineList api`() {
	...
	@Test
	fun `productLinesModifyProductLine with all fields`() {
		val res = createWithAllFields()
		val savedRes = repository.save(res)
	...
	@Test
	fun `productLinesUpdateProductLine with required fields`() {
		val res = createWithRequiredFields()
		val savedRes = repository.save(res)
		savedRes.identity.name = "new identity"
		savedRes.identity.description = "new description"
	...
	@Test
	fun `productLinesUpdateProductLine with all fields`() {
		val res = createWithAllFields()
		val savedRes = repository.save(res)
		savedRes.identity.name = "new identity"
		savedRes.identity.description = "new description"
	...
	private fun resourceAsserts(savedResource: ProductLine, result: MvcResult, index: Int = -1) {
		...
		assertEquals(savedResource.owner, getValue(result, "$prefix.owner"))
	...
	private fun createWithRequiredFields(): ProductLine {
	return ProductLine(
				type = null,
				owner = null,
				account = null,
	...
	private fun createWithAllFields(): ProductLine {
		return ProductLine(
				type = "test_enum_value",
				owner = "test string value",
